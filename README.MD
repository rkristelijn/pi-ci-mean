# Introduction
These are my notes to install a full blown MEAN stack (MongoDB, Express, Angular, Node) on the Raspberry Pi. I keep these notes to be able to quickly set up, or reinstall my environment. 
...
In under one hour you will be able to start development on your raspberry pi, starting from NOOBS install.
...
|  Node | v9.5.0 | `node -v` |
|  NPM | 5.6.0 | `npm -v` |
|  angular/cli | 1.6.7 | `ng -v` |
|  Raspbian | GNU/Linux 9 (stretch) | `cat /etc/os-release` |
|  MongoDB |  v3.0.14 | `mongod --version` |
|  Angular |  v5 | `? --version` |

Disclaimer: The above versions can be different from the console output that I copied inline. Please ignore them and look at above table.
...
This project's goal: Create a platform to minimize cost and powerusage and maximize flexibility, performance and usability for a full Javascript boilerplate (MongoDB,Express,Angular,Node)

## What are we building?

Back to [Table of Contents](#table-of-contents)

This project sets up a quick application, using @angular/cli, with a REST API connected to a MongoDB database. 

* On port 3000, Node serves the REST API. To monitor for changes, nodemon is used.
* On port 4200, the front-end is served. To monitor changes, we use webpack that is shipped with angular/cli. 
* Because the front-end cannot access the REST API directly (since this is CORS) we use webpack to proxy for the REST API. We use `ng serve --proxy-config proxy.conf.json`
* To allow webpack to serve the networking, we add `--host 0.0.0.0 --disable-host-check` to the `ng serve command`
* We allow the project's code to be accessed via a samba share, however if you are accessing the code from the web, this is very slow and unstable
* To overcome having to push back and forth code, we use [NoIDE](#noide).

## The project's offering:

Back to [Table of Contents](#table-of-contents)

Via the web using [noide](#noide):

    +---------------------+
    |  noide in chrome    |
    |                     |
    |  front-end in chrome|                                          proxy.conf
    |                     |                                          +-----+
    |  back-end with postman     +------------+   4200               |     |
    +---------------------+      |    Putty   +--------+-----------------------+
    |   Windows 10        +------> ssh tunnel |        |             |     |   |
    |                     |      +------------+   +----v-------------v-----+---v-+
    +---------------------+                       |   ssh  |noide| Node |webpack |
                                                  +------------------------------+       +------------+
...
Todo: [Update Jessie (8) to Stretch (9)](https://github.com/rkristelijn/pi-ci-mean/issues/1). Debian name their releases after [Pixar](https://www.debian.org/doc/manuals/project-history/ch-releases.en.html) characters.
...
What is `SMB`? > The Server Message Block (SMB) protocol is a network file sharing protocol that allows applications on a computer to read and write to files and to request services from server programs in a computer network. The SMB protocol can be used on top of its TCP/IP protocol or other network protocols. Using the SMB protocol, an application (or the user of an application) can access files or other resources at a remote server. This allows applications to read, create, and update files on the remote server. It can also communicate with any server program that is set up to receive an SMB client request. Windows Server 2012 introduces the new 3.0 version of the SMB protocol.[source](https://technet.microsoft.com/en-us/library/hh831795(v=ws.11).aspx)

What is `apt-get`? > APT (the Advanced Packaging Tool) is an evolution of the Debian .deb software packaging system. It is a rapid, practical, and efficient way to install packages on your system. Dependencies are managed automatically, configuration files are maintained, and upgrades and downgrades are handled carefully to ensure system stability. [source](https://www.computerhope.com/unix/apt-get.htm)

What is `sudo`? > sudo ("superuser do") allows a user with proper permissions to execute a command as another user, such as the superuser. [source](https://www.computerhope.com/unix/sudo.htm)

**In Short**:
0. `sudo apt-get update` < trust me, do this first
...
What is `git`? > Git is a version control system for tracking changes in computer files and coordinating work on those files among multiple people. It is primarily used for source code management in software development,[8] but it can be used to keep track of changes in any set of files. As a distributed revision control system it is aimed at speed,[9] data integrity, and support for distributed, non-linear workflows. Git was created by Linus Torvalds in 2005 for development of the Linux kernel, with other kernel developers contributing to its initial development. Its current maintainer since 2005 is Junio Hamano. As with most other distributed version control systems, and unlike most client–server systems, every Git directory on every computer is a full-fledged repository with complete history and full version tracking abilities, independent of network access or a central server. [source](https://en.wikipedia.org/wiki/Git)

We need git for various things: keep track of our code changes, get code bases from the internet and share code to others.
...
[have git or ssh](https://github.com/rkristelijn/pi-ci-mean/issues/2) [remember my password](https://stackoverflow.com/questions/21956750/secure-push-and-pull-with-no-password-git)

### Remember password for github

Beware that saving passwords is not best practice. However if you are doing a lots of git work, it can be annoying to constantly type your github username and password. It is important to understand that skipping the password only works for ssh. You need to clone using ssh instead of https.

First you need to generate a public and private key:

`ssh-keygen -t rsa -b 4096 -C "email_from@github.com"`

Next you need to make sure that your ssh-agent is running

`eval $(ssh-agent -s)`

Now we add the key to the agent

`ssh-add ~/.ssh/id_rsa`

Next we need to add the public key to github, with your browser, navigate to github.com, to your account and open ['SSH and GPG keys'](https://github.com/settings/keys) and paste the contents of `~/.ssh/id_rsa.pub` using:

`cat ~/.ssh/id_rsa.pub` 

### Test by clonign a repo
...
What is `MEAN`? > The term MEAN stack refers to a collection of JavaScript based technologies used to develop web applications. MEAN is an acronym for MongoDB, ExpressJS, AngularJS and Node.js. From client to server to database, MEAN is full stack JavaScript. [source](https://www.sitepoint.com/introduction-mean-stack/)

What is `Express`? > Express is lightweight framework used to build web applications in Node. It provides a number of robust features for building single and multi page web application. Express is inspired by the popular Ruby framework, Sinatra.
...
The following installs MongoDB 2.4; `sudo apt-get install mongodb-server`. This is not what we want, because both the latest versions of `mongoose` and `mongodb` that are used to connect Node to MongoDB will start whining the can't connect. So we have two options: install earlier versions of `mongoose`, e.g. 4.x or update MongoDB 3.

If you want to install MongoDB 3.x you have two options:

1. Install from scratch, deal with: creating a group, a user, putting all in right directory, make sure it runs for the right user, make sure it starts at boot
2. Patch the MongoDB 2.4 installation that takes care of all above

I go for 2, here's how it's done;

`sudo apt-get install mongodb-server`

Right away I see `mongod` running in `top`. Cool. Also after `sudo reboot` (don't do it now [a]) it's there, so it's starting automatically and we see it also listed in `sudo netstat -ltpn`.

Now we need to replace the MongoDB 2.4 binaries with the 3.x versions.

From [this tutorial](http://andyfelong.com/2017/08/mongodb-3-0-14-for-raspbian-stretch/) we can find the url for the latests binaries for ARM Raspbian Stretch

`pi@rpi2:~ $ wget http://andyfelong.com/downloads/mongodb_stretch_3_0_14_core.zip`

From here we can simply extract the 3 binaries using:

`unzip mongodb_stretch_3_0_14_core.zip`

Before you can toch the binaries, you need to stop the mongod service: `sudo service mongodb stop` (I did tell you to not restart right? [a])

If you want to keep the 2.4 versions: rename them:

`sudo service mongodb stop` - if mongodb was running

`sudo mv /usr/bin/mongo /usr/bin/mongo2.4`

`sudo mv /usr/bin/mongod /usr/bin/mongod2.4`

`sudo mv /usr/bin/mongos /usr/bin/mongos2.4`

Now we move the 3.x versions:

`mv mongo* /usr/bin/`

Now you can start the service if it was not started already, by a reboot or
...
What is `Node`? > Node.js is a server side JavaScript execution environment. It’s a platform built on Google Chrome’s V8 JavaScript runtime. It helps in building highly scalable and concurrent applications rapidly.
...
We want the latest stable node and we use the instructions from [this howto](http://thisdavej.com/beginners-guide-to-installing-node-js-on-a-raspberry-pi/#install-node). Raspbian allows us to add PPAs to our repo so we can get binaries that did not yet passed the Raspbian tests. This does not mean that PPAs are not stable. Testing packages that ship with Raspbian take a lot of time.

What is `PPA`? > PPA stands for personal package archives. It allows us to install the latest version of any software in Linux Mint or Ubuntu from launchpad by adding to repository.
...
Check if we are now going to install the latest version:

`apt list nodejs`

nodejs/onbekend 9.5.0-1nodesource1 armhf
...
Todo: [update Angular 4 to 5](https://github.com/rkristelijn/pi-ci-mean/issues/3)

Back to [Table of Contents](#table-of-contents)

What is `AngularJS`? > AngularJS (commonly referred to as "Angular.js" or "AngularJS 1.X") is a JavaScript-based open-source front-end web application framework mainly maintained by Google and by a community of individuals and corporations to address many of the challenges encountered in developing single-page applications. The JavaScript components complement Apache Cordova, the framework used for developing cross-platform mobile apps. It aims to simplify both the development and the testing of such applications by providing a framework for client-side model–view–controller (MVC) and model–view–viewmodel (MVVM) architectures, along with components commonly used in rich Internet applications. In 2014, the original AngularJS team began working on Angular (Application Platform). [source](https://en.wikipedia.org/wiki/AngularJS)

What is `Angular`? > Angular (commonly referred to as "Angular 2+" or "Angular 2") is a TypeScript-based open-source front-end web application platform led by the Angular Team at Google and by a community of individuals and corporations to address all of the parts of the developer's workflow while building complex web applications. Angular is a complete rewrite from the same team that built AngularJS. Angular is a framework for building client applications in HTML and either JavaScript or a language like TypeScript that compiles to JavaScript. Angular combines declarative templates, dependency injection, end to end tooling, and integrated best practices to solve development challenges. Angular empowers developers to build applications that live on the web, mobile, or the desktop. [source](https://en.wikipedia.org/wiki/Angular_(application_platform))

Version | Released | Changelog |
--------|----------|----------|
Angular 1 | | Originally, the rewrite of AngularJS was called "Angular 2" by the team, but this led to confusion among developers. To clarify, the team announced that separate terms should be used for each framework with "AngularJS" referring to the 1.X versions and "Angular" without the "JS" referring to versions 2 and up.|
Angular 2 | April 30, 2015 | Angular 2.0 was announced at the ng-Europe conference 22-23. September 2014. The drastic changes in the 2.0 version created considerable controversy among developers. On April 30, 2015, the Angular developers announced that Angular 2 moved from Alpha to Developer Preview. Angular 2 moved to Beta in December 2015, and the first release candidate was published in May 2016. The final version was released on September 14, 2016.|
Angular 3 | skipped | |
Angular 4 | December 13, 2016 | On 13 December 2016 Angular 4 was announced, skipping 3 to avoid a confusion due to the misalignment of the router package's version which was already distributed as v3.3.0. The final version was released on March 23, 2017. Angular 4 is backward compatible with Angular 2 |
Angular 5 | November 1, 2017 | Angular 5 was released on November 1, 2017. Key improvements in Angular 5 include support for progressive Web apps, a build optimizer and improvements related to Material Design. Angular 6 release will be pushed back to March or April 2018, with Angular 7 showing up in September or October 2018. Each version is expected to be backward-compatible with the prior release. Google pledged to do twice-a-year upgrades. |

Now because we have npm, we can easily install Angular 4 or greater using:

What is `NPM`? > NPM is a package manager for Node.js packages, or modules if you like. www.npmjs.com hosts thousands of free packages to download and use. The NPM program is installed on your computer when you install Node.js. [see](https://www.npmjs.com/)

What is `angular/cli`? > Angular/cli used to be called angular-cli. So don't install angular-cli, but @angular/cli. When creating angular applications manualy, you need to do a lot of command-line work, creating files and config-files and let them all work together. With angular/cli you can do this with one command. The big advantage of this is, that the folder structure and filenames are standardized so everyone works with the same structure. That makes life easier.
...
Well, that's it. You can now use ng (angular/cli) to create your front-end project (`ng new`), use yo (Yeoman: `sudo npm install -g yo`) to create your Node app or look at my project for smallest mean app.

Note that I first used ng new to create an application and then copied server.js and the api/ folder into the same application, merging the package.json file. You can find my simple project on github. The instructions are in the next chapter.
...
* ng serve monitors file changes and rebuilds/restarts automatically on save.
...
The above commands don't return a console, so you need to open new putty sessions. If you are ok with that, seeing the live update in the putty screens, you are done. If not, please read the Serving chapter. After the Serving chapter [NoIDE](#noide) is discussed. If you don't want to serve `node` and `angular` from `forever` you can go directly to the `noide` part. From `noide` you can serve both the REST API back-end and the front-end.
...
What is `noide`? > No IDE is an IDE written in Node that access (read, write, create, delete, rename) your files from a browser. Next to that you can execute shell commands, such as starting the back-end, front-end, running tests, linting etc. So you need only putty with some setting and you are good to go.

What is `IDE`? > IDE - Integrated Development Environment, that has code highlighting, autocompletion, code checks, and integration with source code controle.

First we need to install forever if you havn't already.
...
This only pull the code from the web. You need to install the dependacies that are described in `package.josn`

`npm install`

Start noide from `~/lean-mean` using: `forever start -c "sudo node ../noide/index.js" ./`

Note that you can also install the original noide from [here](http://davidjamesstone.github.io/noide/) however I created a fork because .ts files are not highlighted properly. Next to that the editor was updated from 1.2.3 to 1.2.9. 
...
|   | Baremetal RPi3 | QEMU | QEMU Slower factor |
|  ------ | ------ | ------ | ------ |
...
What if you don't have a pi, what if you can't connect to your pi and you only have Windows and you still want to get your own dev environment? You can using these steps:

## Oracle VM VirtualBox
[This is a wonderful tutorial](http://www.aoakley.com/articles/2017-07-04-raspbian-x86-virtualbox.php) about installing Raspbian Stretch x86 on Virtual Box, including the guest addition and sharing a folder. But for the sake of completeness, here are my notes. 

First you need to download your software: [VirtualBox](https://www.virtualbox.org/wiki/Downloads), I choose Windows hosts v 5.2.6. Note, when you install, the installation tries to elevate rights. I needed to rightclick the installation file and choose 'Install as Administrator'.

After the installation, you can get an x86 iso file to boot your machine from [downloads.raspberrypi.org](http://downloads.raspberrypi.org/rpd_x86/images/). Note here, that you need the x86-build binary and the arm-versions won't work without QEMU.
...
After creation:
  - Settings
    - Storage
      - IDE: (navigate to iso file downloaded above)
      - flag: Live CD/DVD
  - Network
    ...
    - Advanced
      - Port forwarding
        ...
        - Host IP: 127.0.0.1
        - Host Port: 2222
        - Guest IP: 10.0.2.15 (or any other gotten from `ifconfig | grep inet`)
        - Guest port: 22

Let's see the performance on Windows 10 Enterprise Intel(R) Core(TM) i5-7300U CPU @ 2.60Ghz 2.70Ghz / 8GB ram 64 bit
    ...
    Running MEMORY test...
    Operations performed: 3145728 (3000384.46 ops/sec)
    3072.00 MB transferred (2930.06 MB/sec)
    total time: 1.0484s
    min: 0.00ms
    avg: 0.00ms
    max: 15.76ms
    ...
    Running DD WRITE test...
    536870912 bytes (537 MB, 512 MiB) copied, 2.09196 s, 257 MB/s
    bash: line 59: vcgencmd: command not found

    Running DD READ test...
    536870912 bytes (537 MB, 512 MiB) copied, 0.919696 s, 584 MB/s
    bash: line 64: vcgencmd: command not found


Also it is possible to run a raspberry pi from a NAS using an [x86 ISO](http://downloads.raspberrypi.org/rpd_x86/images/rpd_x86-2017-06-23/). 
    ...
    Running MEMORY test...
    Operations performed: 3145728 (598066.15 ops/sec)
    3072.00 MB transferred (584.05 MB/sec)
    total time: 5.2598s
    min: 0.00ms
    avg: 0.01ms
    max: 20.14ms
    ...
    Running DD WRITE test...
    536870912 bytes (537 MB) copied, 5.36062 s, 100 MB/s

    Running DD READ test...
    536870912 bytes (537 MB) copied, 3.23972 s, 166 MB/s

    AikonCWD's rpi-benchmark completed!

A comparisson table:

|   | Baremetal RPi3 | x86 ISO VM |
|  ------ | ------ | ------ | 
|  CPU total time (s) |  17.4972 | 10.5138 | 
|  CPU min (ms) | 6.96 | 0.93 | 
|  CPU avg (ms) | 6.99 | 4.20 | 
|  CPU max (ms) |  25.9 | 182.78 | 
|  THREADS total time (s) | 11.9219 | 33.7064 | 
|  THREADS min (ms) | 4.61 |1.79 | 
|  THREADS avg (ms) | 4.77 | 13.48 | 
|  THREADS max (ms) | 24.01 | 159.39 |
|  MEMORY total time (s) | 2.4931 | 5.2598 |
|  MEMORY min (ms) | 0.00 | 0.00 | 
|  MEMORY avg (ms) | 0.00 | 0.01 | 
|  MEMORY max (ms) | 8.06 | 20.14 | 
